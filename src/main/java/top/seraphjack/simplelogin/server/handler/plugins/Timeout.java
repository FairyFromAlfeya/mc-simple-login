...
public final class Timeout implements HandlerPlugin {
    ...
    public void preLogin(ServerPlayer player, Login login) {
        ScheduledFuture<?> future = executor
            .schedule(
                ...
                SLConfig.SERVER.secs.get(),
                TimeUnit.SECONDS
            );

        Optional
            .ofNullable(futures.put(login.name, future))
    ...
    public void postLogin(ServerPlayer player, Login login) {
        Optional
            .ofNullable(futures.remove(login.name))
            .ifPresent(f -> f.cancel(true));
    }
    ...
    public void preLogout(ServerPlayer player) {
        Optional
            .ofNullable(futures.remove(player.getGameProfile().getName().toLowerCase()))
            .ifPresent(f -> f.cancel(true));
    }
